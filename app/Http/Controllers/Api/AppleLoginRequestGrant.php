...
class AppleLoginRequestGrant extends AbstractGrant
{
    ...
    protected function getUserEntityByRequest(Request $request)
    {
        ...
        if (method_exists($model, 'loginApple')) {
            $user = (new $model)->loginApple($request);
        } else {
            throw OAuthServerException::serverError('Unable to find loginApple method on user model.');
        }
